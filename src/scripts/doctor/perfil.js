...
function createDynamicElements() {
    ...
    const backdropClass = 'hidden fixed inset-0 bg-gray-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4';
    ...
    toastContainer.className = 'hidden fixed bottom-5 right-5 flex items-center w-full max-w-xs p-4 text-white rounded-lg shadow';
...
const profileAvatar = document.getElementById('profile-avatar'),
    ...
    cancelPasswordChangeBtn = document.getElementById('cancel-password-change-btn'),
...
function showToast(message, type = 'success') {
    ...
    toastIcon.innerHTML = type === 'success' ? `<i class="bi bi-check-circle-fill"></i>` : `<i class="bi bi-exclamation-triangle-fill"></i>`;
    setTimeout(() => toast.classList.add('hidden'), 3000);
}

function populateSelectWithOptions(selectId, options, valueKey, textKey, selectedValue = null) {
    ...
    options.forEach(optionData => {
        ...
        if (optionData[valueKey] !== undefined && optionData[textKey] !== undefined) {
...
function populateCheckboxes(containerId, options, valueKey, textKey, selectedValues = []) {
    const container = document.getElementById(containerId);
    if (!container) return;
    container.innerHTML = '';
    if (!Array.isArray(options) || options.length === 0) {
        container.innerHTML = '<p class="text-gray-500 col-span-full">No hay especialidades disponibles.</p>';
        return;
    }
    ...
    options.forEach(optionData => {
        ...
        if (Array.isArray(selectedValues) && selectedValues.includes(optionData[valueKey])) {
...
function populateProfileData(user) {
    ...
    const initials = (user.nombres && user.apellidos) ? `${user.nombres.charAt(0)}${user.apellidos.charAt(0)}` : '?';
...
function toggleProfileEditMode(isEditing) {
    ...
    const checkboxesContainer = document.getElementById('especialidades-container');
...
async function initProfilePage() {
    try {
        ...
        if (basicUserData.rol_id !== 3) {
             ...
             document.body.innerHTML = '<p class="text-center text-red-500">No tienes permiso para ver esta página.</p>';
             return;
        }
        ...
        populateSelectWithOptions('titulo_profesional_id', titlesData, 'id', 'nombre', doctorData.titulo_profesional_id);
        populateCheckboxes('especialidades-container', specialtiesData, 'id', 'nombre', doctorData.especialidades_ids);
...
profileForm.addEventListener('submit', async (e) => {
    ...
    const checkedSpecialties = document.querySelectorAll('#especialidades-container input[name="especialidades"]:checked');
