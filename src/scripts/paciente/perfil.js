...
function populateProfileData(user) {
  // Normalizar fecha de nacimiento para el input date (YYYY-MM-DD)
  originalProfileData = {
    ...user,
  ...
  Object.keys(originalProfileData).forEach((key) => {
    // Busca el elemento por ID. Es importante que el ID del HTML coincida con la clave del JSON (backend)
...
function toggleProfileEditMode(isEditing) {
  profileInputs.forEach((input) => {
    // El DNI nunca se edita
    if (input.id !== 'dni') {
      input.disabled = !isEditing;
      input.classList.toggle('bg-gray-50', !isEditing);
      // Si es un select, quitamos la clase que impide clicks
      if (input.tagName === 'SELECT') {
        input.classList.toggle('pointer-events-none', !isEditing);
      }
    }
  });
...
async function initProfilePage() {
  try {
    ...
    // Asegúrate de que el select en HTML tenga id="genero_id"
    populateSelectWithOptions(
      'genero_id',
      gendersData,
      'id',
...
profileForm.addEventListener('submit', async (e) => {
  ...
  // Convierte FormData a objeto JSON. Solo funciona si los inputs tienen atributo name
