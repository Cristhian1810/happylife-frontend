...
function createDynamicElements() {
    ...
    // --- Modal para Confirmar Eliminación (VERSIÓN CORREGIDA) ---
    ...
    // Creación y adición de los modales al body
...
function populateSelectWithOptions(selectId, options, valueKey, textKey, placeholderText) {
...
function populateProfileData(user) {
    originalProfileData = { ...user, fecha_nacimiento: user.fecha_nacimiento ? user.fecha_nacimiento.split('T')[0] : '' };
    ...
    Object.keys(originalProfileData).forEach(key => {
        ...
        if (element) {
            element.value = originalProfileData[key] === null ? '' : originalProfileData[key];
        }
    });
}
...
async function fetchAPI(url, options = {}) {
    ...
    const response = await fetch(`https://happylife-backend.onrender.com${url}`, { ...defaultOptions, ...options });
...
async function initProfilePage() {
    try {
        ...
        if (userData.rol_id !== 5) { // Asumiendo rol 5 = Paciente
            if (medicalInfoSection) medicalInfoSection.style.display = 'none';
        } else {
            populateSelectWithOptions('grupo_sanguineo', bloodTypesData, 'id', 'nombre', 'No especificado');
        }
        populateSelectWithOptions('genero_id', gendersData, 'id', 'nombre', 'No especificado');
...
// --- EVENTOS PARA EL MODAL DE ELIMINACIÓN (VERSIÓN CORREGIDA) ---
