...
const populateSelects = async () => {
    try {
        ...
        const tituloSelect = doctorForm.querySelector('select[name="titulo_profesional_id"]');
        tituloSelect.innerHTML = '<option value="">Seleccione Título Profesional...</option>';
        titulos.forEach(t => {
            tituloSelect.innerHTML += `<option value="${t.id}">${t.nombre}</option>`;
        });

        const especialidadesContainer = document.getElementById('especialidades-checkboxes');
        ...
        modalError.textContent = "No se pudieron cargar las especialidades y títulos.";
...
const renderTable = (doctores) => {
    ...
    doctores.forEach(doc => {
        ...
        row.innerHTML = `
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">${doc.nombres}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${doc.apellidos}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${doc.especialidades ? doc.especialidades.join(', ') : 'N/A'}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${doc.numero_colegiatura}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${doc.dni}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${doc.email}</td>
            <td class="px-6 py-4 whitespace-nowrap">
                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${doc.esta_activo ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}">${doc.esta_activo ? 'Activo' : 'Inactivo'}</span>
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                <div class="flex items-center justify-center space-x-2">
                    <button class="edit-btn p-2 rounded-full text-blue-100 bg-blue-600 hover:bg-blue-700" title="Editar" data-id="${doc.id}">
                    ...
                    <button class="delete-btn p-2 rounded-full text-red-100 bg-red-600 hover:bg-red-700 disabled:bg-gray-400" title="Desactivar" data-id="${doc.id}" ${!doc.esta_activo ? 'disabled' : ''}>
...
const openModal = (mode, doctorId = null) => {
    ...
    if (mode === 'add') {
        ...
        doctorForm.querySelector('input[name="nombres"]').value = doctorData.nombres;
        doctorForm.querySelector('input[name="apellidos"]').value = doctorData.apellidos;
        doctorForm.querySelector('input[name="dni"]').value = doctorData.dni;
        doctorForm.querySelector('input[name="telefono"]').value = doctorData.telefono || '';
        doctorForm.querySelector('input[name="email"]').value = doctorData.email;
        doctorForm.querySelector('input[name="numero_colegiatura"]').value = doctorData.numero_colegiatura;
        doctorForm.querySelector('select[name="titulo_profesional_id"]').value = doctorData.titulo_profesional_id;
        doctorForm.querySelector('textarea[name="biografia"]').value = doctorData.biografia || '';
        
        const especialidadesCheckboxes = doctorForm.querySelectorAll('input[name="especialidades"]');
        
        fetch(`${API_BASE_URL}/doctores/${doctorId}`)
            .then(res => res.json())
            .then(docDetails => {
                especialidadesCheckboxes.forEach(checkbox => {
                    checkbox.checked = docDetails.especialidades_ids?.includes(parseInt(checkbox.value));
                });
            });
    }
...
const handleFormSubmit = async (event) => {
    ...
    doctorData.especialidades = Array.from(doctorForm.querySelectorAll('input[name="especialidades"]:checked')).map(cb => parseInt(cb.value));

    const id = doctorData.id;
    const method = id ? 'PUT' : 'POST';
    const url = id ? `${API_BASE_URL}/doctores/${id}` : `${API_BASE_URL}/doctores`;
    
    if (method === 'POST' && !doctorData.password) {
         modalError.textContent = 'La contraseña es obligatoria para nuevos doctores.';
         modalError.classList.remove('hidden');
         return;
    }
...
const handleDeleteConfirm = async () => {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/doctores/${doctorIdToAction}`, { method: 'DELETE' });
...
doctorModal.addEventListener('click', (e) => { if (e.target === doctorModal) closeModal(); });
...
searchInput.addEventListener('input', (e) => {
    ...
    const filtered = allDoctorsData.filter(doc => 
        ...
        (doc.especialidades && doc.especialidades.join(' ').toLowerCase().includes(searchTerm))
    );
