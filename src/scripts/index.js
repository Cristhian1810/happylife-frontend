...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const nombresInput = registerForm.querySelector('input[name="nombres"]');
  const apellidosInput = registerForm.querySelector('input[name="apellidos"]');
  const dniInput = registerForm.querySelector('input[name="dni"]');
  const telefonoInput = registerForm.querySelector('input[name="telefono"]');

  const showToast = (message, type = 'success') => {
    const toastContainer = document.getElementById('toast-container');
    const toast = document.createElement('div');
    const baseClasses =
      'p-4 rounded-lg shadow-md text-white text-sm transition-all duration-300 transform';
  ...
  if (nombresInput) nombresInput.addEventListener('input', validateTextInput);
  if (apellidosInput)
    apellidosInput.addEventListener('input', validateTextInput);
  if (dniInput) dniInput.addEventListener('input', validateNumberInput);
  if (telefonoInput)
    telefonoInput.addEventListener('input', validateNumberInput);

  const cargarGeneros = async () => {
    try {
      const response = await fetch(
        'https://happylife-backend.onrender.com/generos'
      );
  ...
  registerForm.addEventListener('submit', async (e) => {
    ...
    try {
      const response = await fetch(
        'https://happylife-backend.onrender.com/pacientes',
        {
          method: 'POST',
  ...
  loginForm.addEventListener('submit', async (e) => {
    ...
    try {
      const response = await fetch(
        'https://happylife-backend.onrender.com/login',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(data),
      ...
      showToast(
        error.mensaje || 'Error de conexión. Inténtalo de nuevo.',
        'error'
      );
    }
  });
  ...
  loginModal.addEventListener(
    'click',
    (e) => e.target === loginModal && closeModal(loginModal, loginModalContent)
  );
